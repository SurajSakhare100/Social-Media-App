...
export const createStory = createAsyncThunk('stories/createStory', async ({ userId, formData }) => {
    const response = await axiosInstance.post(`${storyUrl}/createstory/${userId}`, formData);
...
export const updateStory = createAsyncThunk('stories/updateStory', async ({ userId, storyId, formData }) => {
    const response = await axiosInstance.put(`${storyUrl}/${userId}/${storyId}`, formData);
...
export const deleteStory = createAsyncThunk('stories/deleteStory', async ({ storyId }) => {
    await axiosInstance.delete(`${storyUrl}/${storyId}`);
...
const storiesSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchStories.fulfilled, (state, action) => {
            state.stories = action.payload.stories; // Update stories with fetched data
        })
        ...
        builder.addCase(deleteStory.fulfilled, (state, action) => {
            state.stories = state.stories.filter(story => story._id !== action.payload); // Remove the deleted story
        })
