...
const createFollowers = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!user || !following) {
      return res.status(400).json({ error: "User and following fields are required" });
    }
    if(user===following){
...
const getFollowers = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(new ApiResponse(200, followers, "Followers fetched successfully with user details"));
  } catch (error) {
